...
Supported:
- PDF  → PyMuPDF (fitz) text extraction + Groq Vision for scanned pages
- DOCX → python-docx
- XLSX → openpyxl
- Images (PNG, JPG, JPEG) → Groq Vision OCR
"""
import logging
import io
...
class DocumentParser:
    ...
    For images and scanned PDFs, delegates to GroqService vision OCR.
    """
    ...
    def __init__(self, groq_service=None):
        """
        :param groq_service: GroqService instance for vision OCR (optional but recommended)
        """
        ...
        try:
            ...
            for page_num, page in enumerate(doc):
                ...
                if page_text and len(page_text) > 50:
                    ...
                    ocr_text = self.groq_service.vision_ocr(img_bytes, 'image/jpeg')
    ...
    def parse_image(self, image_bytes, mime_type='image/jpeg'):
        ...
        if not self.groq_service:
            return '[Image OCR unavailable - Groq service not configured]'
        try:
            return self.groq_service.vision_ocr(image_bytes, mime_type)
        except Exception as e:
    ...
    def _vision_fallback(self, file_bytes, mime_type):
        ...
        try:
            ...
            return self.groq_service.vision_ocr(file_bytes[:1024 * 1024], 'image/jpeg')
        except Exception as e:
