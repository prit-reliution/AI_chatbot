...
class OrderValidator:
    ...
    def _validate_and_enrich_single(self, order_data, is_manual=False):
        ...
        # Collapse duplicate lines (only when all fields are equal)
        merged_lines = []
        for line in enriched_lines:
            match = None
            pid = line.get('product_id')
            pname = (line.get('product') or '').strip().lower()
            orig_pname = (line.get('_original_product') or '').strip().lower()
            ...
            if pname and pname != 'unresolved':
                for m_line in merged_lines:
                    ...
                    m_orig_pname = (m_line.get('_original_product') or '').strip().lower()
                    
                    # 1. Compare original product name (before standardizing)
                    if orig_pname != m_orig_pname:
                        continue
                        
                    # 2. Compare product name/id
                    product_match = False
                    if pid and m_pid:
                    ...
                    try:
                        ...
                        if abs(w1 - w2) > 0.0001 or abs(h1 - h2) > 0.0001:
                            continue
                    ...
                    if p1 is not None and p2 is not None:
                        try:
                            if abs(float(p1) - float(p2)) > 0.0001:
                                continue
                        except (ValueError, TypeError):
                            continue
                            
                    # 5. Compare discount
                    d1 = line.get('discount')
                    d2 = m_line.get('discount')
                    if (d1 is None) != (d2 is None):
                        continue
                    if d1 is not None and d2 is not None:
                        try:
                            if abs(float(d1) - float(d2)) > 0.0001:
                                continue
                        except (ValueError, TypeError):
                            continue
                            
                    # 6. Compare tax
                    t1 = str(line.get('tax') or '').strip().lower()
                    t2 = str(m_line.get('tax') or '').strip().lower()
                    if t1 != t2:
                        continue
                        
                    # 7. Compare uom
                    u1 = str(line.get('uom') or '').strip().lower()
                    u2 = str(m_line.get('uom') or '').strip().lower()
                    if u1 != u2:
                        continue
        ...
        # Pop the temporary _original_product key before completing enrichment
        for line in merged_lines:
            line.pop('_original_product', None)
